...
class EuclideanCodebook(nn.Module):
    ...
    def replace_(self, samples, mask):
        ...
        grace_count = max(10.0, float(self.threshold_ema_dead_code + 1))
        self.cluster_size.data.masked_fill_(mask, grace_count)
        self.embed_avg.data[mask] = (
            self.embed.data[mask] * self.cluster_size.data[mask].unsqueeze(1)
        )
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            # updates codebook (embed)
            self.expire_codes_(x) 
            # updates cluster_size
...
class LanguageVectorQuantization(nn.Module):
    ...
    def forward(self, x, n_q: tp.Optional[int] = None):
        ...
        for layer in self.layers[:n_q]:
            quantized_out, indices, loss = layer(residual)  # Get this layer's representation, indices, and loss [64,75]
            # residual = residual - quantized.detach()
            # quantized_out = quantized_out + quantized
            all_indices.append(indices)
            all_losses.append(loss)
...
if __name__ == "__main__":
    ...
    frame_rate = 75 # should be 75/audio_length
    bandwidth = 6.6
    result = vq(x, frame_rate=frame_rate, bandwidth=bandwidth)
    print("Quantized shape:", result.quantized.shape)
    print("Codes shape:", result.codes.shape)
    print("Bandwidth:", result.bandwidth.item())