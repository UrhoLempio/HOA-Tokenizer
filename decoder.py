...
if __name__ == "__main__":
    # Test the backbone with a random input
    backbone = VocosBackbone(input_channels=512, dim=768, intermediate_dim=2304, num_layers=12)
    x = torch.randn(1, 512, 75)  # Batch size 1, 36 channels, 24000 samples (1.5 second at 16kHz)
    #bandwidth_id = torch.tensor([2])  # Example bandwidth ID for conditioning
    output = backbone(x)
    print(output.shape)  # Expected output shape: (1, 24000, 128)
    head = ISTFTHead(dim=768, n_fft=1280, hop_length=320, padding="same")

    audio = head(output)
    print(audio.shape)  # Expected output shape: (1, 24000)
